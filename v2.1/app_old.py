...
def plot(place, seropeak, FirstPeaktDate,  fCases, SecondPeaktDate, sCases,
    ...
    plotData,R1_cases,R1_index,all_end_index,preparedness, seroAlert = createoutputjson(seropeak, FirstPeaktDate,  fCases, SecondPeaktDate, sCases,
                trans, wa, thie, ThirdWaveEmergenceDate , LockdownStartDate, LockdownEndDate, rm, popdens, vaccDays, vaccCover)
...
def advanced_dashboard():
    if request.method == "POST":
        ...
        try:
            ...
            try: wa = float(request.form['wa'])*30
            except: wa = 0.0
            try: checktrans = request.form['checktrans']
            except: checktrans = ""
            try: trans = float(request.form['trans'])/100
            except: trans = 0
