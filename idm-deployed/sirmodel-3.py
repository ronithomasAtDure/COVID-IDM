...
class CovidApp:
    ...
    def get_json(self):        
        ...
        return data,self.R1_cases, self.R_index_final, self.all_end_index_final, self.preparedness_list,self.Serowarning
    ...
    def prep_scenario_dict(self,intervention_state_df,FirstPeakDate,FWP_index, all_end_index ):
        ...
        preparedness_precent=[0.05,0.15,0.85,0.217,0.415,0.621,0.237,0.129,0.142]
        ...
        if len(intervention_state_df.columns)<=9:        
            ...
            if all_end_index[2]!= all_end_index[3]:
                ...
                C1_TW=list(scenario_dict['no_intervention_c1_diff'][all_end_index[2]:all_end_index[3]]) # 10 represnt no. of days patient in hospital
                C1_max_index = C1_TW.index(max(C1_TW))
                preparedness_list.append(sum(C1_TW[C1_max_index-4:C1_max_index+1])*preparedness_precent[0])
                
                C2_TW=list(scenario_dict['no_intervention_c2_diff'][all_end_index[2]:all_end_index[3]])
                C2_max_index = C2_TW.index(max(C2_TW))
                preparedness_list.append(sum(C2_TW[C2_max_index-4:C2_max_index+1])*preparedness_precent[1])
                
                C3_TW=list(scenario_dict['no_intervention_c3_diff'][all_end_index[2]:all_end_index[3]])
                C3_max_index = C3_TW.index(max(C3_TW))
                preparedness_list.append(sum(C3_TW[C3_max_index-4:C3_max_index+1])*preparedness_precent[2])
        ...
        if len(intervention_state_df.columns)>9: 
            ...
            if all_end_index[2]!=all_end_index[3]:
                ...
                C1_TW=list(scenario_dict['no_intervention_c1_diff'][all_end_index[2]:all_end_index[3]]) # 10 represnt no. of days patient in hospital
                C1_max_index = C1_TW.index(max(C1_TW))
                preparedness_temp_C.append(sum(C1_TW[C1_max_index-4:C1_max_index+1])*preparedness_precent[0])
                
                C2_TW=list(scenario_dict['no_intervention_c2_diff'][all_end_index[2]:all_end_index[3]])
                C2_max_index = C2_TW.index(max(C2_TW))
                preparedness_temp_C.append(sum(C2_TW[C2_max_index-4:C2_max_index+1])*preparedness_precent[1])
                
                C3_TW=list(scenario_dict['no_intervention_c3_diff'][all_end_index[2]:all_end_index[3]])
                C3_max_index = C3_TW.index(max(C3_TW))
                preparedness_temp_C.append(sum(C3_TW[C3_max_index-4:C3_max_index+1])*preparedness_precent[2])
                ...
                Cv1_TW=list(scenario_dict['no_intervention_cv1_diff'][all_end_index[2]:all_end_index[3]]) # 10 represnt no. of days patient in hospital
                Cv1_max_index = Cv1_TW.index(max(Cv1_TW))
                preparedness_temp_Cv.append(sum(Cv1_TW[Cv1_max_index-4:Cv1_max_index+1])*preparedness_precent[0]*Cv_effectiveness_percent)
                
                Cv2_TW=list(scenario_dict['no_intervention_cv2_diff'][all_end_index[2]:all_end_index[3]])
                Cv2_max_index = Cv2_TW.index(max(Cv2_TW))
                preparedness_temp_Cv.append(sum(Cv2_TW[Cv2_max_index-4:Cv2_max_index+1])*preparedness_precent[1]*Cv_effectiveness_percent)
                
                Cv3_TW=list(scenario_dict['no_intervention_cv3_diff'][all_end_index[2]:all_end_index[3]])
                Cv3_max_index = Cv3_TW.index(max(Cv3_TW))
                preparedness_temp_Cv.append(sum(Cv3_TW[Cv3_max_index-4:Cv3_max_index+1])*preparedness_precent[2]*Cv_effectiveness_percent)
    ...
    def run_scenario(self,user_input_dict):
        ...
        FirstPeakDate = datetime.datetime.strptime('16-9-2020', '%d-%m-%Y').date() if user_input_dict['FWP_date'] == '' else datetime.datetime.strptime(user_input_dict['FWP_date'], '%d-%m-%Y').date()
        self.SecondPeakDate = datetime.datetime.strptime('16-9-2020', '%d-%m-%Y').date() if user_input_dict['SWP_date'] == '' else datetime.datetime.strptime(user_input_dict['SWP_date'], '%d-%m-%Y').date()
        ...
        self.ThirdWaveEmergenceDate =None if user_input_dict['TW_Emergence_date'] == '' else datetime.datetime.strptime(user_input_dict['TW_Emergence_date'], '%d-%m-%Y').date()
        self.LockdownStartDate = None if user_input_dict['LockdownStartDate'] == '' else datetime.datetime.strptime(user_input_dict['LockdownStartDate'], '%d-%m-%Y').date()
        self.LockdownEndDate =None if user_input_dict['LockdownEndDate'] == '' else datetime.datetime.strptime(user_input_dict['LockdownEndDate'], '%d-%m-%Y').date()
        ...
        self.proportionHospital= user_input_dict['proportionHospital']
        self.proportionOxygen= user_input_dict['proportionOxygen']
        self.homeCare= user_input_dict['homeCare']
        self.percentIncOxygenhomeCare=0.05
        # FWEindextemp=400
       ...
       ################## Passing parameter to Sirmodel #########################################################################################
        ...
        evaluation = optimize_FWE_R2(solution)
        
        if evaluation>=0.1:
            self.Serowarning='yes'
...
class SirModel:
    ...
    def ThirdWaveEndlimit (self,state_arr,vaccflag=None, Wa=None, lckdn=None):
        ...
        if vaccflag==None:
            if Wa==0:        
                ThirdDuration=state_Cdiff_max_index*2.5
