...
def plot(place, r1, r2, r3, wa, thie, FirstPeaktDate,ThirdWaveEmergenceDate, LockdownStartDate, LockdownEndDate, effLck, popdens, vaccDays, vaccCover):
    ...
    restrictiveMeasures = {
        "0":"", "30":"Night Curfew", "40":"Night Curfew & Weekend Curfew",
        "60":"Night Curfew & Halfday Curfew", "80":"Full Day & Night Restriction"
    }
    selectefflck = ["", "", "", "", ""]
    if effLck == 0.3:
        selectefflck[1] = "selected"
    elif effLck == 0.4:
        selectefflck[2] = "selected"
    elif effLck == 0.6:
        selectefflck[3] = "selected"
    elif effLck == 0.8:
        selectefflck[4] = "selected"
    else:
        selectefflck[0] = "selected"
    ...
    values={
        ...
        "Lockdown Start Date":LockdownStartDate, "Lockdown End Date":LockdownEndDate, "Increase in Population":str(popdens*100)+"%",
        "Restrictive Measures":restrictiveMeasures[str(round(effLck*100))], "Vaccine Coverage (Full Dose)":str(vaccCover*100)+"%"
    }
    ...
    return graphJSON, round(R1_cases/10000, 2), SeroDate, wavestatus, waMsg, preparedness, selectefflck
...
def advanced_dashboard():
    if request.method == "POST":
        ...
        return render_template("advanced-dashboard.html", graphJSON=graphJSON, place=place, r1=r1, r2=r2, r3=r3, wa=wa/30, thie=thie*100,
                FirstPeaktDate=FirstPeaktDate, ThirdWaveEmergenceDate=ThirdWaveEmergenceDate, LockdownStartDate=LockdownStartDate,
                LockdownEndDate=LockdownEndDate, effLck=selectefflck, popdens=popdens*100, vaccDays=vaccDays/30,
...
def basic_dashboard():
    if request.method == "POST":
        ...
        #passing all the details to the html
        return render_template("basic-dashboard.html", graphJSON=graphJSON, place=place, r1=r1, r2=r2, r3=r3, wa=wa/30, thie=thie*100,
                FirstPeaktDate=FirstPeaktDate, ThirdWaveEmergenceDate=ThirdWaveEmergenceDate, LockdownStartDate=LockdownStartDate,
                LockdownEndDate=LockdownEndDate, effLck=selectefflck, popdens=popdens*100, checkpop=checkpop, vaccDays=vaccDays/30,
                vaccCover=vaccCover*100, recoveryCases=R1_cases, preparedness=preparedness, wavestatus=wavestatus, waMsg=waMsg, seroDate=seroDate)
